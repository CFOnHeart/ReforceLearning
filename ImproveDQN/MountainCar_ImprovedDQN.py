...
model_name = "./model/mountain_improved_dqn.h5"
train_graph_name = "./graph/mountain_improved_train_dqn.png"
test_graph_name = "./graph/mountain_improved_test_dqn.png"

def train(graph_name, max_episodes=1000, max_iter=2000):
...
def test(graph_name, max_episodes=500, max_iter=2000):
    env = gym.make(env_name)
    env = env.unwrapped
    state_size = env.observation_space.shape[0]
    action_size = env.action_space.n

    agent = DQNAgent(state_size, action_size, batch_size=64, train_start=100, epsilon=0.00001)
